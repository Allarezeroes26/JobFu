...
const Companies = () => {
  const { employers, checkingEmployersList, getAllEmployers } = employeeStore();
  const { jobs, gettingAllJob, getAllJobs } = jobStore();
  ...
  useEffect(() => {
    getAllEmployers();
    getAllJobs();
  }, [getAllEmployers, getAllJobs]);

  const isLoading = checkingEmployersList || gettingAllJob;

  const filteredCompanies = employers
    .filter((c) => c.companyName?.toLowerCase().includes(searchQuery.toLowerCase()))
    .map((c) => ({
      ...c,
      activeJobs: jobs.filter(
        (j) => (typeof j.employer === "string" ? j.employer : j.employer?._id) === c._id
      ).length,
    }));
        ...
        {filteredCompanies.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredCompanies.map((company) => (
              <Card
                key={company._id}
                className="hover:shadow-xl transition-all border-muted-foreground/10 group overflow-hidden"
              >
                ...
                <CardFooter className="p-0">
                  <Link to={`/company/${company._id}`} className="w-full">
                    <Button
                      variant="ghost"
                      className="w-full h-14 rounded-none hover:bg-primary hover:text-white group"
                    >
