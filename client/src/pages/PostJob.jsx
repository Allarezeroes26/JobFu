...
import { Loader2, AlertCircle, Lock, Plus, X } from "lucide-react";
...
const PostJob = () => {
  const { employerProfile, checkingEmployer } = employeeStore();
  ...
  useEffect(() => {
    if (!employerProfile) {
      employeeStore.getState().checkEmployer();
    }
  }, []);
    ...
    return (
      <div className="py-6">
        ...
        <div className="mt-6 flex flex-col items-center justify-center p-8 border border-dashed rounded-lg opacity-60">
          ...
          <p className="text-sm font-medium">Please create or upgrade your account</p>
        </div>
      </div>
    );
  }
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            {formData.requirements.map((req, i) => (
              <Badge key={i} variant="secondary" className="pl-3 pr-1 py-1 gap-1">
                {req}
                <X className="w-3 h-3 cursor-pointer hover:text-destructive" onClick={() => removeRequirement(i)} />
              </Badge>
            ))}
          </div>
        </div>
        
        <Button type="submit" className="w-full h-11" disabled={jobStore().postingJob}>
          {jobStore().postingJob ? "Posting..." : "Create Job Listing"}
        </Button>
      </form>
    </ScrollArea>
  );
